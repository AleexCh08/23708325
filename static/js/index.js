document.addEventListener('DOMContentLoaded', function() {
    ...
    if (path === '/' || path === '/index.html') {
        ...
        cargarPerfilDinamico(perfilId, lang);
    }
    ...
    // Funcion para redirigir desde el index al perfil del estudiante seleccionado (SPA)
    function clickPerfiles() {
        ...
        document.querySelectorAll('.estudiante').forEach(item => {
            item.addEventListener('click', function() {
                ...
                window.history.pushState({}, '', `/perfil?id=${idEstudiante}&lang=${lang}`);
                cargarPerfilDinamico(idEstudiante, lang);

            });
        });
    }

    async function cargarPerfilDinamico(id, lang) {
    try {
        // Cambiar a fetch del endpoint de Python que genera el HTML completo
        const response = await fetch(`/perfil?id=${id}&lang=${lang}`);
        ...
        document.body.innerHTML = `<h1>Error al cargar el perfil: ${error.message}</h1>`;
    }
}
});